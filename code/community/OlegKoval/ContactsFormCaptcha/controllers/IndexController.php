        ...
        if (Mage::getStoreConfigFlag(self::XML_PATH_CFC_ENABLED)) {
            try {
                ...
                if ($post) {
                    ...
                    $captcha = recaptcha_check_answer($privatekey, $remote_addr, $post["recaptcha_challenge_field"], $post["g-recaptcha-response"]);

                    if (!$captcha->is_valid) {
                        throw new Exception($this->__("The reCAPTCHA wasn't entered correctly. Go back and try it again." . print_r($captcha, true)), 1);
                    }
