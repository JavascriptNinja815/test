    ...
    public function getPointsSpent()
    {
        ...
        foreach ($this->_transfers as $transfer) {
            ...
            if ($transfer->getQuantity() < 0) {
                ...
                $revokedTransfers = Mage::getModel('rewards/transfer')
                    ->getTransfersAssociatedWithTransfer($transfer->getId());

                foreach ($revokedTransfers as $revokedTransfer) {
                    if ($revokedTransfer->getQuantity() > 0) {
                        $points += $revokedTransfer->getQuantity();
                    }
                }
            }
        }
