 ...
 */
class TBT_Rewards_Block_Manage_Widget_Loyalty extends Mage_Adminhtml_Block_Template {
...
FEED;
	...
	protected function _getLoyaltyUrl() {
	    ...
	    $url_data = array();
	    $url_data["a"] = "rewards";
	    $url_data["v"] = (string) Mage::getConfig()->getNode('modules/TBT_Rewards/version');
	    $url_data["m"] =  Mage::getVersion();
	    $url_data["p"] =  urlencode($this->getBaseUrl());
	    $url_data["ap"] =  urlencode($this->getAction()->getFullActionName());
	    $url_data["license"] =  Mage::helper('rewards/loyalty')->getLicenseKey();
