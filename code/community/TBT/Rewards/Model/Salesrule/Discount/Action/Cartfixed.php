    ...
    protected function _getTotalFixedDiscountOnitem($item, $address, $rule, &$cartRules) {
        ...
        if (is_array($this->_rewardsRulesItemTotals[$rule->getId()])) {
            ...
            if ($this->_rewardsRulesItemTotals[$rule->getId()]['items_count'] <= 1) {
                ...
                $discountRate = ($this->_rewardsRulesItemTotals[$rule->getId()]['base_items_price'] !== 0) 
                    ? $this->_getItemBasePrice($item) * $this->_getItemQty($item, $rule) / ($this->_rewardsRulesItemTotals[$rule->getId()]['base_items_price'])
                    : 0;
