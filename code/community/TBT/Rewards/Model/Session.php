...
class TBT_Rewards_Model_Session extends Mage_Core_Model_Session_Abstract {
    ...
    public function getTotalPointsSpentOnCart($cart = null) {
        ...
        $pointsObj = new Varien_Object(array('points' => $points, 'points_exist' => $points_exist, 'force_positive' => true));

        Mage::dispatchEvent('rewards_points_spent_on_cart_additional', array('points_obj' => $pointsObj, 'quote' => $cart, 'items' => $cart->getAllItems()));

        $points = $pointsObj->getPoints();
        $points_exist = $pointsObj->getPointsExist();
