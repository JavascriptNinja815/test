 ...
 */
class TinyBrick_OrderArchive_Block_Adminhtml_Sales_Order_Grid extends Mage_Adminhtml_Block_Sales_Order_Grid
{
    ...
    // CH comes back    
        ...
        ->joinLeft(array('orderarchive_tbl'=>Mage::getSingleton('core/resource')->getTableName('orderarchive')),
        		'orderarchive_tbl.order_id = main_table.entity_id',
        		'order_group_id')
        		->where('orderarchive_tbl.`order_group_id` IS NULL OR orderarchive_tbl.`order_group_id`=0');
  ...
  'filter_index'     => 'main_table.store_id',
    ...
    protected function _prepareMassaction()
    {
        ...
        if(Mage::getStoreConfig('toe/orderarchive/activate_archive_orders')) {
			$this->getMassactionBlock()->addItem('archive_order', array(
					'label'=> Mage::helper('sales')->__('Archive Order(s)'),
					'url'  => $this->getUrl('orderarchive/adminhtml_orderarchive/archiveorder'),
			));
        }
        
        if(Mage::getStoreConfig('toe/orderarchive/activate_delete_orders')) {
        	$this->getMassactionBlock()->addItem('delete_order', array(
        			'label'=> Mage::helper('sales')->__('Delete Order(s)'),
        			'url'  => $this->getUrl('orderarchive/adminhtml_orderarchive/deleteorder'),
        			'confirm' => Mage::helper('sales')->__('Are you sure? Order(s) will be permanately deleted!') // -- Add confirm JS box to be sure of user action
        	));
        }   
