...
class TinyBrick_OrderEdit_Model_Order_Address extends Mage_Sales_Model_Order_Address
{
    ...
    {
        return Mage::getModel('orderedit/order_address_rate')->getCollection()
            ->addFieldToFilter('address_id', $this->getId())
            ;
    }
	...
	{
        //mage::Log(__METHOD__ . __LINE__ . " recalc ");
        ...
        $request->setPackageWeight($this->getPackageWeight());
    ...
    {
        $real_order_id = Mage::app()->getRequest()->getParam('order_id');

        if($real_order_id) {
            $order = Mage::getModel('sales/order')->load($real_order_id);
        } else {
            $order = $this->getOrder();
        }
        ...
        foreach($order->getAllItems() as $item) {
            ...
            $qty = (int)$item->getQty() ? $item->getQty() : (int)$item->getQtyOrdered();
            //mage::log(__METHOD__ . __LINE__ . " we have weight ". $item->getWeight() ."  aND qty " . $qty);
           $weight += $item->getWeight() * $qty;
        }
        //mage::log(__METHOD__ . __LINE__ . " we have weight ". $weight);
        return $weight;
    }
