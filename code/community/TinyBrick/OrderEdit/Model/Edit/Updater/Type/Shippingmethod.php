...
class TinyBrick_OrderEdit_Model_Edit_Updater_Type_Shippingmethod extends TinyBrick_OrderEdit_Model_Edit_Updater_Type_Abstract
{
	...
	{	
        ...
        // begin collinsharper changes
        // don't overwrite the original shipping amount; an adjustment line will be added to the order
        // set new* fields for later use
		if($data['customPrice'] != '') {
			//$order->setShippingAmount($data['customPrice']);
            //$order->setBaseShippingAmount($data['customPrice']);
			$order->setNewShippingAmount($data['customPrice']);
            $order->setNewBaseShippingAmount($data['customPrice']);
		} else {
			if($data['rateid'] != 'custom') {
				//$order->setShippingAmount($shippingRate->getPrice());
				$order->setNewShippingAmount($shippingRate->getPrice());
			}
		}
        // end collinsharper changes

		try{
			$order->save();
			$newMethod = $order->getShippingDescription()." - $".substr($order->getNewShippingAmount(),0,-2);
