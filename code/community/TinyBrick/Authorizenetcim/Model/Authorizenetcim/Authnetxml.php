...
class TinyBrick_Authorizenetcim_Model_Authorizenetcim_Authnetxml extends Varien_Object
{
    ...
    {
        $storeId = false;
        if(isset($storeId['store'])) {
            $storeId  = $storeId['store'];
        }
        $testMode = Mage::getStoreConfig('payment/authorizenetcim/test_mode',$storeId);
        if($testMode){
            $login = Mage::getStoreConfig('payment/authorizenetcim/test_username',$storeId); //trim($login);
            $transkey = Mage::getStoreConfig('payment/authorizenetcim/test_password',$storeId); //trim($transkey);
            $test = self::USE_DEVELOPMENT_SERVER;
        }
        else{
            $login = Mage::getStoreConfig('payment/authorizenetcim/username',$storeId); //trim($login);
            $transkey = Mage::getStoreConfig('payment/authorizenetcim/password',$storeId); //trim($transkey);
    ...
    private function process()
    {
        ...
        mage::log(__METHOD__ . " we have api call "   . " wioth data " . print_r($this->xml,1));
        $helper = new TinyBrick_Authorizenetcim_Helper_Data;

        // Clean up the request first
        $nodes = @new SimpleXMLElement($this->xml);
        unset($nodes->profile->paymentProfiles->payment->creditCard->cardNumber);
        unset($nodes->profile->paymentProfiles->payment->creditCard->cardCode);
        $helper->debugLog(__FILE__, __LINE__, print_r($nodes->asXML(), true));
        ...
        {
            $helper->debugLog(__FILE__, __LINE__, print_r($this->response, true));
