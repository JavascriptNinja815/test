...
class TinyBrick_Authorizenetcim_IndexController extends Mage_Core_Controller_Front_Action
{
	...
	/**
	 * Deletes the credit cared on file
	 */
	...
	/**
	 * submits the new credit cared to put on file
	 */
	public function submitAction(){
		...
		if(!$params['edit'] && strlen($ccNumber) >= 10){
			...
			$profileUpload = Mage::getModel('authorizenetcim/authorizenetcim');
			$profileUpload->load($ccId);
			$profileUpload->setCcType($ccType);
			$profileUpload->setCcLast4(substr($ccNumber, -4, 4));
			$profileUpload->setCcExpMonth(substr($ccExp, -2));
			$profileUpload->setCcExpYear(substr($ccExp, 0, -3));
			$profileUpload->save();
			...
			$profileUpload->setCcExpMonth(substr($ccExp, -2));
			$profileUpload->setCcExpYear(substr($ccExp, 0, -3));
			$profileUpload->setVerification($ccVer);
			$profileUpload->save();
		}
		$this->_redirectSuccess(Mage::getUrl('*/*/index', array('_secure'=>true)));
