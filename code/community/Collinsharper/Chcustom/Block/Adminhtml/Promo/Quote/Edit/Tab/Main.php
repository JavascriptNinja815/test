...
class Collinsharper_Chcustom_Block_Adminhtml_Promo_Quote_Edit_Tab_Main extends Mage_Adminhtml_Block_Promo_Quote_Edit_Tab_Main
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset->addField('notes', 'textarea', array(
            'name' => 'notes',
            'label' => Mage::helper('salesrule')->__('Notes'),
            'title' => Mage::helper('salesrule')->__('Notes'),
            'style' => 'height: 100px;',
        ));


        $fieldset->addField('myretail', 'select', array(
            'label'     => Mage::helper('salesrule')->__('MyReel Retail'),
            'title'     => Mage::helper('salesrule')->__('MyReel Retail'),
            'name'      => 'myretail',
            'options'   => array(
                '1' => Mage::helper('salesrule')->__('Yes'),
                '0' => Mage::helper('salesrule')->__('No'),
        ...
        if($model->getId() && $model->getMyretail() && !$model->getLabelsPrinted()) {

            $chUrl =  Mage::helper("adminhtml")->getUrl('chcustom_admin/index/printlabels', array('id' => $model->getId()));

            $continueButton = $this->getLayout()
                ->createBlock('adminhtml/widget_button')
                ->setData(array(
                'label'     => Mage::helper('widget')->__('Print Labels'),
            //    'onclick'   => "setSettings('".$this->getContinueUrl()."', 'type', 'package_theme')",
                'onclick'   => "chPrintLabels('" . $chUrl . "')",
                'class'     => 'save'
            ));
            $fieldset->addField('continue_button', 'note', array(
                'text' => $continueButton->toHtml() . "<br /> <small>You can only download the PDF for the labels once. Ensure to save / print it at that time.</small>",
            ));
