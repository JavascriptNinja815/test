...
class Mage_Sales_Model_Resource_Report_Order_Collection extends Mage_Sales_Model_Resource_Report_Collection_Abstract
{
    ...
    protected function _getSelectedColumns()
    {
        ...
        } elseif ('hour' == $this->_period) {
            $this->_periodFormat = $adapter->getDateFormatSql('period', '%Y-%m-%d %H:00:00');
        } else {
     ...
     */
    public function setDateRange($from = null, $to = null, $start_time = null, $end_time = null)
    {
        $this->_from = $from . (($start_time) ? ' ' . $start_time : '');
        $this->_to   = $to . (($end_time) ? ' ' . $end_time : '');
