        ...
        if($this->reel_filter) {
            $this->getSelect()
                ->joinInner(
                    ...
                    array('order_items.product_options'))
                ->where('order_items.product_options LIKE ?', '%s:' . strlen('Reel ID') . ':"Reel ID";s:5:"value";s:' . strlen($this->reel_filter) . ':"' . $this->reel_filter . '";%')
                ->group('main_table.entity_id');
        }
mage::log(__METHOD__ .  __LINE__ . " we have sql " . $this->getSelect()->__toString());
    ...
    public function getSize()
    {
        ...
        if($this->reel_filter) {
            if (is_null($this->_totalRecords)) {
                $recordCounts = $this->getConnection()->fetchAll($sql, $this->_bindParams);
                $this->_totalRecords = count($recordCounts);
            }
        }
        else {
