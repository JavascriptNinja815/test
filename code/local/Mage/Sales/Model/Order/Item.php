    ...
    public function getProductOptions()
    {
        if ($options = $this->_getData('product_options')) {
			...
			foreach($optionsArr['options'] as $x => $option) {
				if($option['label'] == 'Reel Image') {
					unset($optionsArr['options'][$x]);
				}

				if($option['label'] == 'Reel Name') {
					$has_name = 1;
				}
			}
			...
			foreach($optionsArr['options'] as $option) {
				...
				if($option['label'] == 'Reel ID') {
					$reel = Mage::getModel('chreels/reels')->load($option['value']);
					if($reel && !$has_image && strpos(Mage::helper('core/url')->getCurrentUrl(), '/print/') === FALSE && strpos(Mage::helper('core/url')->getCurrentUrl(), '/api/') === FALSE) {
                        $thumb = $reel->getThumb() ? Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . '/'  . $reel->getThumb() : $this->getMediaUrl('reel_builder_templates/customer_reel_default.jpg');
                        $name = $reel->getReelName();
                        $image = "javascript:showReelPreview('{$thumb}', '{$name}')";
                        $value = "<a href=\"{$image}\" title=\"{$name}\"><img src=\"{$thumb}\" width=\"75\" alt=\"{$name}\" /></a>";
					...
					if($reel && !$has_name) {
						$newOptions[] = array(
							'label' => 'Reel Name',
							'value' => $reel->getData('reel_name'),
							'print_value' => $reel->getData('reel_name'),
							'option_id' => '',
							'option_type' => '',
							'custom_view' => false
						);
...
function my_urlencode_item($str) {
