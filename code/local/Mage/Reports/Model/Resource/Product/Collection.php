    ...
    public function addOrderedQty($from = '', $to = '')
    {
        ...
        $this->getSelect()->reset()
            ->from(
                ...
                array(
                    'order_ids' => 'GROUP_CONCAT(order_items.order_id)',
    ...
    function _afterLoad() {
	    ...
	    if(!empty($items)) {
			...
			foreach($items as $x => $item) {
				$order_ids = $item->getOrderIds();
				if($order_ids) {
					$order_array = explode(',', $order_ids);
					$order_array = array_unique($order_array);
					foreach($order_array as $order_id) {
						$order_items_used = array();
						$order = Mage::getModel('sales/order')->load($order_id);
						$theItems = $order->getAllVisibleItems();
						if(!empty($theItems)) {
							foreach($theItems as $theItem) {
								$theSku = $theItem->getSku();
								if($theItem->getProductId() == $item->getEntityId()) {
									if(!$newItemTally[$theSku]) {
										$newItemTally[$theSku]['qty'] = 0;
										$newItemTally[$theSku]['obj'] = $item;
									}
									$newItemTally[$theSku]['qty'] += $theItem->getQtyOrdered();
								}
							}
						}
					}
			...
			if(!empty($newItemTally)) {
				foreach($newItemTally as $tmpSku => $tmpItem) {
					$newItem = clone $tmpItem['obj'];
					$newItem->setOrderedQty($tmpItem['qty']);
					$newItem->setSku($tmpSku);
					$newItem->setOrderItemsName($newItem->getOrderItemsName() . ' (' . $tmpSku . ')');
