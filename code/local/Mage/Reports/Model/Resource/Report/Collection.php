...
class Mage_Reports_Model_Resource_Report_Collection
{
     ...
     * @return Mage_Reports_Model_Resource_Report_Collection
     */
    public function setInterval($from, $to, $start_time = '00:00:00', $end_time = '23:59:59')
    {
        $this->_from = $from;
        $this->_to   = $to;
            ...
            while ($dateStart->compare($dateEnd) <= 0) {
                ...
                switch ($this->_period) {
                    case 'day':
						...
						if($dateStart->toString('yyyy-MM-dd') == date('Y-m-d', strtotime($this->_from))) {
	                        $t['start'] = $dateStart->toString('yyyy-MM-dd ' . $this->_start_time);
						}
						else {
							$t['start'] = $dateStart->toString('yyyy-MM-dd 00:00:00');
						}
						if($dateStart->toString('yyyy-MM-dd') == date('Y-m-d', strtotime($this->_to))) {
	                        $t['end'] = $dateStart->toString('yyyy-MM-dd ' . $this->_end_time);
						}
						else {
