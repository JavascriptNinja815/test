...
{
    const REPORT_PERIOD_TYPE_HOUR    = 'hour';
    ...
    public function getIntervals($from, $to, $period = self::REPORT_PERIOD_TYPE_DAY)
    {
        ...
        if ($period == self::REPORT_PERIOD_TYPE_DAY || $period == self::REPORT_PERIOD_TYPE_HOUR) {
            $dateStart = $start;
        }
        ...
        while ($dateStart->compare($dateEnd) <= 0) {
            switch ($period) {
                case self::REPORT_PERIOD_TYPE_HOUR :
	                $t = $dateStart->toString('yyyy-MM-dd HH:mm:ss');
	                $dateStart->addHour(1);
	                break;
