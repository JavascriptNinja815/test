...
class Mage_Checkout_Block_Cart_Item_Renderer extends Mage_Core_Block_Template
{
    ...
    public function getProductImage()
    {
		...
		return (string)Mage::getBaseUrl().'media/catalog/product'.$product->getImage();
    }
        ...
        if ($optionIds = $this->getItem()->getOptionByCode('option_ids')) {
            ...
            foreach (explode(',', $optionIds->getValue()) as $optionId) {
                if ($option = $this->getProduct()->getOptionById($optionId)) {
                    ...
                    $reelId = Collinsharper_Image3d_Model_Observer::REELNAMEOPTIONID == $option->getId() ? $quoteItemOption->getValue() : false;
                    $reel = Mage::getModel('chreels/reels')->load($reelId);


                    if(Collinsharper_Image3d_Model_Observer::REELNAMEOPTIONID == $option->getId() && $reelId && $reel) {

                        $options[] = array(
                            'label' => 'Reel Name',
                            'value' => $group->getFormattedOptionValue($reel->getData('reel_name')),
                            'print_value' => $group->getPrintableOptionValue($reel->getData('reel_name')),
                            'option_id' => $option->getId(),
                            'option_type' => $option->getType(),
                            'custom_view' => $group->isCustomizedView(),
                            'id_value' => $group->getFormattedOptionValue($quoteItemOption->getValue())
                        );
                    }
                    else {
                        $options[] = array(
