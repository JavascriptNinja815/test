...
class AW_Giftcard_Model_Giftcard extends Mage_Core_Model_Abstract
{
    ...
    public function setRvPrefix()
    {
	$this->_prefix = self::RV_PREFIX;
    }
    ...
    {
    	$giftCardParams = Mage::getSingleton('core/session')->getGiftcardParams();
    	if (isset($giftCardParams['prefix_code']) && $giftCardParams['prefix_code']) {
    		$customPrefix = '-' . strtoupper($giftCardParams['prefix_code']) . '-';
    	} else {
    		$customPrefix = '';
    	}
	...
	if ($this->_prefix != '') {
        ...
        if ($customPrefix && strtolower($prefix) == 'rv') {
            $code = "{$prefix}{$customPrefix}{$code}{$suffix}";
        } else {
