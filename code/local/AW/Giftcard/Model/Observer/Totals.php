...
class AW_Giftcard_Model_Observer_Totals extends Mage_Core_Model_Abstract
{
    public function coreBlockAbstractToHtmlAfter(Varien_Event_Observer $observer)
    {
	    ...
	    $observer->getTransport()->setHtml($block->toHtml() . $currentHtml);
        }
        return $this;
    }
        ...
        if (count($quoteGiftcardsItems) > 0) {
            foreach ($quoteGiftcardsItems as $card) {
                ...
                $giftcardModel
                    ...
                    ->save()
                ;
                $_gift_cards[] = array( 'i' => $card->getGiftcardId(),
                                        'c' => $card->getCode(),
                                        'a' => (double)$card->getBaseGiftcardAmount(),
                                        'ba' => (double)$card->getBaseGiftcardAmount(),
                                        'authorized' => (double)$card->getBaseGiftcardAmount()
                );

                $base_gift_cards_invoiced = $base_gift_cards_invoiced + $card->getBaseGiftcardAmount();
            }

            // KL: Update the sales_flat_order
            $sale = Mage::getModel('sales/order')->load($order->getId());
            $sale->setData('gift_cards', serialize($_gift_cards));
            $sale->setData('base_gift_cards_amount', $base_gift_cards_invoiced);
            $sale->setData('gift_cards_amount', $base_gift_cards_invoiced);
            $sale->setData('base_gift_cards_invoiced', $base_gift_cards_invoiced);
            $sale->setData('gift_cards_invoiced', $base_gift_cards_invoiced);
            $sale->save();
        }
        return $this;
    }
