...
class AW_Giftcard_Block_Adminhtml_Giftcard_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	...
	$collection->getSelect()->joinLeft(
		...
		'history.giftcard_id = main_table.entity_id',
		array('balance_amount')
		)->columns(array(
			'issue_amount' => 'history.balance_amount'
		))->where("history.action = '" . AW_Giftcard_Model_Source_Giftcard_History_Action::CREATED_VALUE . "'");
    ...
    protected function _prepareColumns()
    {
		...
		$this->addColumn('redemption_code_cost', array(
            'header'        => $this->__('Redemption Code Cost'),
	...
	$this->addColumn('issue_amount', array(
            'header'        => $this->__('Purchase Price'),
            'currency_code' => Mage::app()->getStore()->getBaseCurrency()->getCode(),
            'type'          => 'number',
            'renderer'      => 'aw_giftcard/adminhtml_widget_grid_column_renderer_currency',
            'index'         => 'issue_amount',
