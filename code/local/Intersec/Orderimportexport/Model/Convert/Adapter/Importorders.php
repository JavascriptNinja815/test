...
//        }
                                       ...
                                       $website_id = "")
    {
        ...
        if ($shipping_regions) {
            ...
            foreach ($shipping_regions as $region_data) {

                $region1_id = $region_data->getId();
        ...
        $addressData = array(
            ...
            "country_id" => $this->getCountryCode($country_id),
        ...
        $addressData2 = array(
            ...
            "country_id" =>  $this->getCountryCode($shipping_country_id),
        ...
        $this->log("customerId:$customerId");
...

    {
        ...
        //This array is to set the Required "REEL ID" custom field for each sku
        $reelIdOptionNumbers = array(
            'VIEWER' => '9',
            'REEL' => '11',
            'IMPRINT' => '43'
        );
...
/*
        $lstorders = Mage::getModel('sales/order')->getCollection()->addAttributeToFilter('increment_id', array('gteq' => 28880000000));
        $lstorders->getSelect()->order('increment_id DESC')->limit('1');
        $lastOrderId = $lstorders->getFirstItem()->getIncrementId();
*/

        //update increment ID for store entity

        if (0 && isset($importData['store_id']) && isset($importData['order_id']) && $orderTypeId != "" && $importData['order_id'] != "" && is_numeric($importData['order_id'])) {
        ...
        foreach ($products_ordered as $data) {
            ...
            $this->log("adding  product" . $product->getName());

            if (!$product || !$product->getId()) {

                $this->log(__METHOD__ . __LINE__ . " MISSING PRDUCT SOMETHING IS WRONG");
            ...
            try {
                ...
                $this->log(sprintf('Order #' . $importData['order_id'] . ' saving error: %s', $e->getMessage()));
            ...
            #$this->log "OTHER ID: ". $product->getId();

            $reelIdOptionKey = $add_products_array[$product->getId() . "-" . $parts[2]]['options_details']['0'];

            $add_products_array[$product->getId() . "-" . $parts[2]]['qty'] = $parts[1];
            $add_products_array[$product->getId() . "-" . $parts[2]]['options'][$reelIdOptionNumbers[$reelIdOptionKey]] = "rg-item_" . $product->getId() . "-" . $parts[2];
//            $add_products_array[$product->getId() . self::PRODUCT_SEPERATOR . $productcounter]['qty'] = $parts[1];

            $productcounter++;

        }

        $this->log(__METHOD__ . __LINE__ . "add product array: " . print_r($add_products_array, 1));
        ...
        $this->log(__METHOD__ . __LINE__ ." grrrr " .  $final_shipping['regionid'] . " and " . $customer->getDefaultBillingAddress()->getRegion() . " " . $customer->getDefaultBillingAddress()->getPostcode() );

        if(is_numeric($final_shipping['regionid']) &&
            $final_shipping['postcode'] == $customer->getDefaultBillingAddress()->getPostcode()) {
            $final_shipping['region'] = $customer->getDefaultBillingAddress()->getRegion();
            $final_shipping['regionid'] = $customer->getDefaultBillingAddress()->getRegion();
        }


        //assemble data structure

   //     $this->log(__METHOD__ . __LINE__ . " we have add array " . print_r($add_products_array, 1));
        ...
        $this->log(__METHOD__ . __LINE__ . " we have ORDER DAGTA  " . print_r($orderData, 1));
        ...
        if (!empty($orderData)) {
            ...
            $this->processOrder($orderData, $importData, $write, $prefix, $products_ordered, $read, $lastOrderId);
    ...
    public function processOrder(&$orderData, $importData, $write, $prefix, $products_ordered, $read, $lastOrderId)

    {
        ...
        try {
            ...
            try {
               ...
               // 20150808 - force increment id !!!
               $order1->setIncrementId($importData['order_id']);
            ...
            if (method_exists($order1, 'getId')) {
                ...
                $this->log(sprintf('Order #' . $importData['order_id'] . ' ERROR SAVING ORDER SKIPPING ROW'), null, 'ce_order_import_errors.log');
                exit;
            }
...
//                $option->save();
            ...
            // begin resetting increment id
            /* 20151206 - not needed
            $orderTypeId = Mage::getModel('eav/entity')->setType('order')->getTypeId();
            $prefix = Mage::getConfig()->getNode('global/resources/db/table_prefix');
            $resource = Mage::getSingleton('core/resource');
            $write = $resource->getConnection('core_write');
            $sql ="UPDATE `" . $prefix . "eav_entity_store` SET increment_last_id = " . $lastOrderId . " WHERE store_id = " . "1" . " AND entity_type_id = " . $orderTypeId . "";
            $this->log($sql );
            $write->query($sql);
            // end resetting increment id
            */
            ...
            $this->log('Order Successfull');

        } catch (Exception $e) {

            #$this->log(sprintf('Order saving error: %s', $e->getMessage()), Zend_Log::ERR);
    ...
    public function getCountryCode($country){
        ...
        $country_array = mage::getModel('directory/country_api')->items();
        ...
        foreach($country_array as $c){
            ...
            }elseif ( $c['country_id'] == $country ) {

                return $c['country_id'];

            }
        }
        //id wasn't found so we'll return what was passed in
        return $country;
        ...
        if ($customer->getId()) {
            ...
            if ($this->getBatchParams('update_customer_address') == "true") {
                ...
                $dataBilling = array(
                    ...
                    'region' => $billing_region,
                    'region_id' => $region,
                    'country_id' => $this->getCountryCode($importData['billing_country']),
                ...
                $dataShipping = array(
                    ...
                    'region_id' => $region1,
                    'country_id' => $this->getCountryCode($importData['shipping_country']),
                ...
                try {
                    ...
                    $this->log(sprintf('ERROR Billing Address Not Updated: ' . $e->getMessage(), ''), null, 'ce_order_import_errors.log');
                }
                ...
                try {
                    ...
                    $this->log(sprintf('ERROR Shipping Address Not Updated: ' . $e->getMessage(), ''), null, 'ce_order_import_errors.log');
                }
            ...
            $this->log(__METHOD__ . __LINE__ . " we have SH " . $importData['shipping_region'] . " BI " .  $importData['billing_region']);
...

    {
        ...
        $productArrayKeyIdentifier = $product->getId(). "-" . $parts[2];
        ...
        $this->log(__METHOD__ . __LINE__ . print_r($parts, 1));


        if (method_exists($product, 'getTypeId')) {

            $part_type = isset($parts[$partsPosition['producType']]) ? $parts[$partsPosition['producType']] : 'simple';
...
//            }
            ...
            $isStockAvailable = $productstockItem->getStockItem()->getIsInStock() &&
                $productstockItem->getStockItem()->getManageStock() == 1 &&
                $stockItem->getQty() != "0.0000" || $productstockItem->getStockItem()->getManageStock() == 0;
            if ($isStockAvailable) {

                $this->log(__METHOD__ . __LINE__);

                //if ($productstockItem->getStockItem()->getIsInStock() && $productstockItem->getStockItem()->getManageStock() == 1 && $stockItem->getQty() != "0.0000" || $productstockItem->getStockItem()->getManageStock() == 0 || ($part_type == "bundle" && $productstockItem->getStockItem()->getIsInStock())) {

                if ($product->getTypeId() == "simple" && $part_type == "simple") {

                    $this->log(__METHOD__ . __LINE__ . ' WE MADE IT ');
                    ...
                    if (!empty($options)) {
                        $this->log(__METHOD__ . __LINE__);
                        list($comparisonSku, $junk)  = explode("-", $parts[$partsPosition['optionSku']]);

                        foreach ($options as $option) {

                            $this->log(__METHOD__ . __LINE__ . " optoin " . print_r($option->getData(),1));

                            #echo "TYPE: " . $option->getType();
                        //    if($option->getSku() == $parts[$partsPosition['optionSku']]) {
                            if ($option->getTitle() == 'Viewer Color' || $option->getTitle() == 'Reel ID') {
                                ...
                                if ($option->getType() == "drop_down" ||
                                    ...
                                    foreach ($values as $value) {
                                      ...
                                      //  $this->log(__METHOD__ . __LINE__ . " VALUE DATA " . print_r($value->getData(), 1));
                                        $this->log(__METHOD__ . __LINE__ . " comapring {$value->getSku()} and {$comparisonSku} ");

                                        if ($value->getSku() == $comparisonSku) {
...
//
                    ...
                    $add_products_array[$productArrayKeyIdentifier]['options'] = $simple_custom_option_values;
                    $add_products_array[$productArrayKeyIdentifier]['options_details'] = $parts;

                  //  $this->log(__METHOD__ . __LINE__ . " we have optoins! " . print_r($simple_custom_option_values, 1));
                }
                ...
                if ($product->getTypeId() == "configurable" && $part_type == "configurable") {
                    ...
                    $add_products_array[$productArrayKeyIdentifier]['super_attribute'] = $super_attribute_order_values;
                    ...
                    if ($stockItem->getQty() > 0 || $product->getTypeId() == "bundle") {
                        ...
                        if ($product->getTypeId() == "bundle" && $part_type == "bundle") {
                            //TODO: must account for proper sku identifier in sku array. BROKEN
                        ...
                        if ($part_type == "configurable") {
                            ...
                            if ($productcounter == 1) {
                                ...
                                $add_products_array[$productArrayKeyIdentifier]['qty'] = $parts[1];
                        ...
                        #$this->log(sprintf('Order #' . $importData['order_id'] . ' PRODUCT OUT OF STOCK'), null, 'ce_order_import_errors.log');
               ...
               //     $this->log(__METHOD__ . __LINE__ . " adding custom product ! " . print_r($parts, 1));
            ...
            $this->log(sprintf('ERROR PRODUCT NOT EXIST: [sku]' . $parts[0], ''), null, 'ce_order_import_errors.log');
 ...
 //TODO: must account for proper sku identifier in sku array. BROKEN
        ...
        $orderData = array(
            ...
            'order' => array(
                ...
                'shipping_address' => array(
                 ...
                 $this->log(__METHOD__ . __LINE__ . " SHIP DATA " . print_r($final_shipping, 1));

        //  $this->log(__METHOD__ . __LINE__ . " we have prod data " . print_r($orderData, 1));
        return $orderData;
...

    {
        ...
        try {
          //  $this->log(__METHOD__ . __LINE__ . " imrtoing now CLASS " . get_class($this->_getOrderCreateModel()));
       // 20150808 - sharper Collins Harper = force increment_id
	// 20151117 mess with orde rnumber
       $orderData['order']['increment_id'] = isset($orderData['order']['order_id']) ? $orderData['order']['order_id'] : $orderData['payment']['po_number'];
            ...
            echo "ERROR Saving Order createOrder(): " . $e->getMessage();

            #Mage::throwException(Mage::helper('catalog')->__('Order saving error: %s', $e->getMessage()));

            #$this->log(sprintf('Order saving error: %s', $e->getMessage()), Zend_Log::ERR);

            $this->log(sprintf('Order #' . $importData['order_id'] . ' saving error: %s', $e->getMessage()), null, 'ce_order_import_errors.log');
...

    {
        ...
        try {
            ...
            $this->log(sprintf('ERROR With Payment: %s', $e->getMessage()), null, 'csv_order_import_errors.log');
    ...
    { //Create a new Invoice for the order
        ...
        try {
            ...
            $this->log("failed to create an invoice");

            $this->log($e->getMessage());

            $this->log($e->getTraceAsString());

            $this->log(sprintf('failed to create an invoice: %s', $e->getMessage()), null, 'ce_order_import_errors.log');
            ...
            try {
                ...
                if (method_exists($product2, 'getTypeId')) {
                    ...
                    try {
                        ...
                        $this->log(sprintf('Order #' . $importData['order_id'] . ' ERROR UPDATING PRODUCT PRICE: %s', $e->getMessage()), null, 'ce_order_import_errors.log');
                ...
                if ($productexistsletsnotupdateprice != true) {
                    ...
                    $write_qry2 = $write->query("
                                            UPDATE
                                                `" . $prefix . "sales_flat_order_item`
                                            SET
                                                price_incl_tax = '" . $custom_row_total . "',
                                                base_price_incl_tax = '" . $custom_row_total . "',
                                                base_row_total_incl_tax = '" . $custom_row_total . "',
                                                row_total_incl_tax = '" . $custom_row_total . "',
                                                row_total = '" . $order_total_without_tax . "',
                                                base_row_total = '" . $order_total_without_tax . "',
                                                tax_amount = '" . $tax_percent_for_row_total . "',
                                                base_tax_amount = '" . $tax_percent_for_row_total . "',
                                                original_price = '" . $custom_row_total . "',
                                                base_original_price = '" . $custom_row_total . "',
                                                price = '" . $custom_row_total . "',
                                                base_price = '" . $custom_row_total . "'
                                            WHERE
                                                order_id = '" . $order1->getId() . "'
                                                AND sku = '" . $productskutocheck . "'");
                }
                ...
                if ($this->getBatchParams('create_invoice') == "true") {
                    ...
                    if ($productexistsletsnotupdateprice != true) {
                        ...
                        $write_qry3 = $write->query("
                        UPDATE
                            `" . $prefix . "sales_flat_invoice_item`
                        SET
                            price_incl_tax = '" . $custom_row_total . "',
                            base_price_incl_tax = '" . $custom_row_total . "',
                            base_row_total_incl_tax = '" . $custom_row_total . "',
                            ...
                            tax_amount = '" . $tax_percent_for_row_total . "',
                            base_tax_amount = '" . $tax_percent_for_row_total . "',
                            price = '" . $order_total_without_tax . "',
                            base_price = '" . $order_total_without_tax . "'
                        WHERE
                            parent_id = '" . $invoice_entity_id . "'
                                AND sku = '" . $productskutocheck . "'");
                    }
                ...
                if ($configurableskusomatchonlike) {
                    ...
                    $write_qry2 = $write->query("
                                            UPDATE
                                                `" . $prefix . "sales_flat_order_item`
                                            SET
                                                price_incl_tax = '" . $partsfornamepricing[1] . "',
                                                base_price_incl_tax = '" . $partsfornamepricing[1] . "',
                                                base_row_total_incl_tax = '" . $order_total_has_tax . "',
                                                row_total_incl_tax = '" . $order_total_has_tax . "',
                                                row_total = '" . $order_total_without_tax . "',
                                                base_row_total = '" . $order_total_without_tax . "',
                                                tax_amount = '" . $tax_percent_for_row_total . "',
                                                base_tax_amount = '" . $tax_percent_for_row_total . "',
                                                original_price = '" . $order_total_without_tax . "',
                                                base_original_price = '" . $order_total_without_tax . "',
                                                price = '" . $order_total_without_tax . "',
                                                base_price = '" . $order_total_without_tax . "'
                                            WHERE
                                                order_id = '" . $order1->getId() . "'
                                                AND sku LIKE '%" . $productskutocheck . "%'");
                } else {
...

    {
        ...
        if (isset($parts[3]) && $parts[2] != "configurable" && $parts[2] != "bundle" && $parts[2] != "simple") {
            ...
            try {
                ...
                $this->log(sprintf('Order #' . $importData['order_id'] . ' ERROR UPDATING PRODUCT NAME: %s', $e->getMessage()), null, 'ce_order_import_errors.log');
    ...
    { //Create new Shipment for the order
        ...
        try {
            ...
            $this->log("failed to create a shipment");

            $this->log($e->getMessage());

            $this->log($e->getTraceAsString());

            $this->log(sprintf('failed to create a shipment: %s', $e->getMessage()), null, 'ce_order_import_errors.log');
...

    {
        ...
        if (isset($parts[3]) && $parts[2] != "configurable" && $parts[2] != "bundle" && $parts[2] != "simple") {
            ...
            try {
                ...
                $this->log(sprintf('Order #' . $importData['order_id'] . $message), null, 'ce_order_import_errors.log');
            ...
            try {
                ...
                $write->query(
                    ...
                    "UPDATE `" . $prefix . "sales_flat_order_item` "
                        ...
                        . "SET price_incl_tax = original_price"

                        . ", base_price_incl_tax = base_original_price"

                        . ", base_row_total_incl_tax = '" . $order_total_with_tax

                        . "', row_total_incl_tax = '" . $order_total_with_tax

                        . "', row_total = '" . $importData['grand_total']

                        . "', row_total_incl_tax = '" . $importData['grand_total']

                        . "', base_row_total = '" . $importData['grand_total']

                        . "', base_row_total_incl_tax = '" . $importData['grand_total']
...
//
//                        . ", original_custom_price = original_price"
//
//                        . ", price_incl_tax = original_price"
//
//                        . ", base_price_incl_tax = original_price"
//
//                        . ", row_total_incl_tax = '" . $order_total_with_tax
//
//                        . "', base_row_total_incl_tax = '" . $order_total_with_tax
...
//                            . "' AND sku = '" . $parts[0] . "'");
...

    {
        ...
        if ($verChecksplit[1] > 4) {
            ...
            try {
                ...
                $this->log(sprintf('Order #' . $importData['order_id'] . ' saving error: %s', $message), null, 'ce_order_import_errors.log');
    ...
    {
        Mage::log($data, null, "CH_importexport.log");
    }
