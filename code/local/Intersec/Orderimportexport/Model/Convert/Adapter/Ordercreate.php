...
class Intersec_Orderimportexport_Model_Convert_Adapter_Ordercreate extends Varien_Object //Intersec_Orderimportexport_Model_Convert_Adapter_Importorders
{
     ...
     * @return  Mage_Adminhtml_Model_Sales_Order_Create
     */
    public function addProduct($product, $qty=1, $data = false)
    {
        $qty = (float)$qty;
        ...
        if ($item = $this->getQuote()->getItemByProduct($product)) {
            ...
            $product->setSkipCheckRequiredOption(true);
            $item = $this->getQuote()->addProduct($product, $qty, $data['options']);
//            $item = $this->getQuote()->addProduct($product, $qty, $options);
            $product->unsSkipCheckRequiredOption();
            $item->checkData();
        }
        ...
        if(isset($data['options']) && $data['options'] != false) {
            ...
            $optionSku  = $data['options_details'][2];
            mage::log(__METHOD__ . __LINE__ . " we have optoin sku " . $optionSku);

            mage::log(__METHOD__ . __LINE__ . " weehave options " . print_r($options, 1));
            $optionIds = array_keys($options);
            $item->addOption(new Varien_Object(
                array(
                    'product' => $product,
                    'code' => 'option_ids',
                    'value' => implode(",",array_keys($options)) /* Option id goes here. If more options, then comma separate */
                )
            ));

            foreach($options as $optionId => $optionValue) {

                mage::log(__METHOD__ . __LINE__ . " weehave options  $optionId => $optionValue " );
            ...
            if($item->getSku() == 'REEL') {
                $item->setSku('REEL-' . $optionSku);
            }

            if($item->getSku() == 'VIEWER') {
                $item->setSku('VIEWER-' . $optionSku);
            }

            mage::log(__METHOD__ . __LINE__ . "we have something " . print_r($item->getSku(),1));
    ...
    {
        mage::log(print_r($products, 2), null, "productsArrayOrderCreate.log");
        foreach ($products as $productIdentifier => $data) {
            $options = false;
            list($productId, $productIdentifierOption) = explode("-", $productIdentifier, 2);
            ...
            try {
                $this->addProduct($productId, $qty, $data);
            }
...
//                }
   ...
   //    Mage::log(__METHOD__ . __LINE__ ." we have  data " . print_r($data,1));
