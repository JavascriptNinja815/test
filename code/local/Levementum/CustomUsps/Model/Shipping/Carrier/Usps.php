 ...
 */
class Levementum_CustomUsps_Model_Shipping_Carrier_Usps
    extends Mage_Usa_Model_Shipping_Carrier_Usps
{
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        //KL: Check to see if we had total qty more then 38? and it is for international customer?
        ...
        if ($origCountry != $destCountry) {
            ...
            if ($request->getAllItems()) {
                ...
                if ($total_qty > $this->getConfigData('allowed_qty')) {
                    return false;
                }
            }
        }
                     ...
                     */
                        ...
                        if (is_object($xml->Package) && is_object($xml->Package->Service)) {
                            foreach ($xml->Package->Service as $service) {
                                //echo "Description: ".(string)$service->SvcDescription."<br />";
                                if((string)$service->SvcDescription == "Priority Mail International") {
                                    $SvcDescription = "Priority Mail International (Estimated delivery 1-2 weeks)";
                                }
                                else {
                                    $SvcDescription = (string)$service->SvcDescription;
                                }
                                $serviceName = $this->_filterServiceName($SvcDescription);
                                if($serviceName=="Priority Mail International") $serviceName="Priority Mail International (Estimated delivery 1-2 weeks)";
                                if($serviceName=="First-Class Package International Service") $serviceName="FirstClass Mail International (No tracking, Est. delivery 4-8 weeks)";
                                $serviceCode = 'INT_' . (string)$service->attributes()->ID;
                                $serviceCodeToActualNameMap[$serviceCode] = $serviceName;
                                //echo "Code: ".$serviceCode." Name: ".$serviceName."<br />";
                                if (in_array($serviceCode, $allowedMethods)) {
