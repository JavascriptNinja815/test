...
// magento truncates messages...
  ...
  $hasImageInPost = round(strlen($this->_request_data->source_file)/(1024*1024),4);
        ...
        mage::log("CID {$this->_request_data->customer_id} RID {$this->_request_data->reel_id} FID {$this->_request_data->current_frame_entity_id} FIDX {$this->_request_data->current_frame_index} ACTION {$this->_request_data->action} HASSAVEIMAGE {$hasSaveImage} HASIMAGE {$hasImageInPost} HASTEXT {$hasTextInPost} ", null, 'missing_frame_image_trace.log', true);
        return $this->$action();
