...
class Collinsharper_Beanstreamprofiles_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getStoreConfig('payment/beanstreamprofiles/debug');
    }

    public function getTest()
    {
        return Mage::getStoreConfig('payment/beanstreamprofiles/test');
    }
    ...
    public function getCcAvailableTypes()
    {
        ...
        $availableTypes = Mage::getStoreConfig('payment/beanstreamprofiles/cctypes');
        if ($availableTypes) {
    ...
    public function getAddressesHtmlSelect($type = 'billing', $prefixUpdate = 'billing', $addOption = false)
    {
        if ($this->isCustomerLoggedIn()) {
            ...
            $select = Mage::app()->getLayout()->createBlock('core/html_select')
                ->setName($prefixUpdate.'_address_id')
                ->setId($prefixUpdate.'-address-select')
...
if($addOption) {
    ...
    public function getReciept()
    {
        ...
        if('beanstreamprofiles' != (string)$order->getPayment()->getMethod())
        {
            return false;
        }
    ...
    {
        return Mage::getSingleton('core/resource')->getTablename('collinsharper_beanstreamprofiles');
    }
    ...
    {
        $table = Mage::getSingleton('core/resource')->getTablename('collinsharper_beanstreamprofiles_transaction');
    ...
    public function getTokenDataSinFromBs($token_id, $trnId)
    {
        ...
        $data = array();
        $std = new Collinsharper_Beanstreamprofiles_Model_PaymentMethod;
...
//		curl_setopt($ch, CURLOPT_POSTFIELDS,);
    ...
    public function getProducts($order)
    {
        ...
        if(Mage::getStoreConfig('payment/beanstreamprofiles/force_decline'))
        {
            $test = '{TESTD}';
        }
        $verification = 0;
    ...
    public function saveReceipt($_d)
    {
        ...
        $order->addStatusToHistory(
            ...
            Mage::helper('beanstreamprofiles')->__(' Beanstream Profiles CC Payment results, '.$_d)
        );
        $order->save();
    }
}
